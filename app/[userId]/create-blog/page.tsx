...
const Page = () => {
  ...
  const addBlogHandler = async ()=>{
    const postData = {...blogData, categories: ctg, postedBy: authUser.user._id}
    try{
        ...
        <ul className="flex text-sm gap-2 flex-wrap">
            {
                ctg?.map( (ctg, i)=>(
                    ...
                    className={`select-none relative border px-2 py-1 rounded-lg ${clrs[i%(clrsLength)].border} ${clrs[i%(clrsLength)].bg}`}>
                      {ctg}
                      <i onClick={()=>(removeCtg(ctg))} className={`fi fi-sr-circle-xmark absolute -right-2 -top-2 cursor-pointer text-${clrs[i%(clrsLength)].text}`} />
                    </li>
                    )
                )
            }
